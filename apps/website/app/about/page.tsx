...
const pillars = [
  ...
  },
  {
    ...
    title: 'Securing',
    description:
      "Every rand is tracked. Every payment is audited. Every mandate is verified. The platform protects the collective's money with the same rigour a bank would.",
...
const values = [
  ...
  },
  {
    ...
    body: 'Every contribution is a brick in a future built intentionally. We exist not just for today, but to create generational financial security.',
  },
  {
    icon: Handshake,
...
const platformFeatures = [
  { heading: 'Invite-only access',     body: 'Membership is by invitation only. Every member is verified before they join.' },
  { heading: 'Automated debits',       body: 'Monthly contributions are collected via Netcash DebiCheck — zero manual transfers.' },
  { heading: 'Full audit trail',       body: 'Every action is logged. Every rand is traceable. Every admin decision is recorded.' },
  { heading: 'Goal tracking',          body: 'Set collective financial goals, track progress in real time, and celebrate every milestone.' },
  { heading: 'Instant notifications',  body: 'SMS, email, and push alerts keep every member informed of debits, reminders, and more.' },
  { heading: 'PDF statements',         body: 'Download formal payment statements for any period, ready for records or tax purposes.' },
]
...
export default function AboutPage() {
  return (
    <>
      ...
      <main className="pt-[calc(var(--nav-height)+36px)]">

        {/* ── Hero ──────────────────────────────────────────────────── */}
        <section className="relative overflow-hidden bg-xxm-green py-20 md:py-28 px-4">
          <div
          ...
          <div className="relative max-w-screen-md mx-auto text-center flex flex-col items-center gap-6">
            ...
            <div className="w-8 h-8 rounded-full bg-xxm-gold/20 flex items-center justify-center mx-auto">
            ...
            <p className="text-white/80 text-base italic leading-relaxed max-w-sm">
              ...
              <cite className="text-xxm-gold/70 text-xs not-italic block mt-1 tracking-wide">
                — Acts 20:35
              </cite>
            </p>
        ...
        <section className="py-16 md:py-24 px-4 bg-white">
          <div className="max-w-screen-md mx-auto">
            ...
            <div className="space-y-5 text-gray-600 text-[15px] leading-relaxed max-w-prose mx-auto">
              ...
              <p>
                Rooted in the age-old African wisdom of ubuntu, Xkimm Xa Mali was conceived as
                something more than ordinary group savings: a structured, transparent,
                technology-powered platform that takes the principle of collective financial
                discipline and gives it the rigour, accountability, and automation of a proper
          ...
          <div className="max-w-screen-lg mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {pillars.map(({ icon: Icon, title, description, accent, iconBg }) => (
                <div key={title} className={`rounded-2xl border p-7 flex flex-col gap-5 ${accent}`}>
        ...
        <section className="py-16 md:py-24 px-4 bg-white">
          <div className="max-w-screen-lg mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {founders.map(({ initials, name, title, bio, initBg, ring }) => (
                <div
                  key={name}
                  className={`bg-white rounded-2xl border border-xxm-gold/10 p-6 flex flex-col sm:flex-row gap-5 ring-2 ${ring} hover:shadow-gold transition-all duration-300`}
                >
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-xl font-black shrink-0 ${initBg}`} aria-hidden>
                    {initials}
                  </div>
        ...
        <section className="py-16 md:py-24 px-4 bg-white">
          <div className="max-w-screen-md mx-auto text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left mb-10">
              {platformFeatures.map(({ heading, body }) => (
                <div key={heading} className="rounded-xl bg-xxm-champagne-100 border border-xxm-gold/10 p-4">
