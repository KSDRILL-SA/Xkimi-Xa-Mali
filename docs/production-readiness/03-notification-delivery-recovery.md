...
|---|---|---|
| Real count: 229 permanently failed (115 SMS, 114 EMAIL) — **not** 200 (101/99) | `PASSED` (measured, root cause fixed, and now fully resolved) | `SELECT channel, count(*) FILTER (WHERE status='FAILED' AND "retryCount">=3) ... GROUP BY channel` against the real `notifications` table. **The original "200" figure was an artifact of the alert's own query, not the true count** — `countAbandonedNotifications()` capped its scan at `take: 200`, so once the true backlog crossed 200 the alert simply stopped counting higher. **Fixed**: replaced with a real `GROUP BY` count (`notificationRepo.countByChannel`) that can never be truncated. **Also since discovered (see the corrected row below): all 229 were the system's own internal `admin-alert-*` notifications, addressed to the owner, not member communications** — the incident sounded worse than it was. All 229 are now resolved: 115 EMAIL sent successfully once Resend was fixed, 114 SMS deleted as stale once confirmed non-member-facing (1 of the original 115 SMS was sent successfully as a controlled test first). |
...
| The 115 SMS rows | `PASSED` (resolved — not what it looked like) | **Correction, 2026-08-29 night, checked directly against the DB:** these 115 (114 after the controlled test) were never member-facing payment/overdue notifications. Every single one was `admin-alert-sms` — this system's own internal operational alert ("Job not running: the job heartbeat check"), addressed to the owner's own phone, firing repeatedly (~every 14 min) between 2026-08-27 23:15 and 2026-08-29 02:30 while BulkSMS was unconfigured, so the alert *about* the problem also failed to send. The equivalent 114 EMAIL rows recovered earlier this session were the same thing (`admin-alert-email`) — confirmed by checking what actually sent once Resend was fixed: 115 `admin-alert-email` + 1 unrelated `founder-badge-granted`, zero member-facing templates. Nothing was ever owed to a member. The underlying job-heartbeat issue is itself resolved (`checks.jobs: "ok"` on the live health endpoint, no new alerts since 02:30). All 114 remaining rows deleted at the owner's request 2026-08-29 once this was confirmed. `notifications` table is now 100% `SENT`, 0 `FAILED`, 0 `QUEUED` — genuinely clean, not just caught up. |
