...
| Admin: manage financial records | `NOT STARTED` | Admin-side money-path console surfaces weren't in the #343 member sweep (that was member-API only) — `docs/session-handoff.md` §8 explicitly lists "the admin console's own surfaces beyond roles" as unaudited. |
...
| Generate statements (Member: own / Admin: authorized) | `NOT STARTED` | Source table's own row — not yet separately probed as an authorization test (i.e. can Member A fetch Member B's statement by ID). Statement *content* is well-audited (§10 below); statement *access control* is not the same thing and hasn't been. |
