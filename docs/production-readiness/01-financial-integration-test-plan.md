...
| # | Test case | Status | Evidence / Notes |
|---|---|---|
| a | Two requests update the same contribution simultaneously | `PASSED` (mechanism, one narrow residual noted) | `recalculateContributionStatus`'s actual write is race-safe by construction, not just by locking: `amountPaid` is always recomputed as a fresh `SUM` over every successful transaction (`transactionRepo.aggregate`) inside an optimistic-retry loop (`updateByVersion` against a `version` column, 3 attempts) — never trusted from a stale local value, so two concurrent writers can never corrupt or lose either payment; both are always correctly reflected. **Narrower residual, not a data-integrity bug**: `submitManualPayment`'s own "does this exceed the remaining balance" guard (`CTR_004`) checks against a `contribution.amountPaid` read *before* the transaction, not re-validated inside it — two concurrent manual payments that are each individually within the remaining balance, but whose sum exceeds it, could both pass and leave the contribution slightly overpaid. Every rand is still correctly recorded on real transaction rows; nothing is lost, miscounted, or double-spent. Left as a documented, low-severity gap rather than a same-night fix, given the two same-shape fixes already made this pass (transaction-webhook race below, and PR #339 for goals) and the very narrow window required to hit it in practice. |
...
| e | Two processes update the same transaction | `PASSED` (found and fixed a real gap) | `processTransactionWebhook` read `transaction.status` outside any lock, then only *later* wrote inside `runTransaction` with a plain unconditional `update` — no guard against a second concurrent webhook delivery reading the same pre-update status and reaching the write too. Exact-duplicate redelivery of the same webhook body is already deduped upstream (`claimWebhookEvent`/`webhookEventKey` in the route), but two distinct events racing on the same transaction were not. Worst case if both won: the post-commit ledger posting (`postPoolCredit`/`postPoolDebit`, outside the DB transaction) would run twice, double-crediting the pool for one real payment. Fixed with a compare-and-swap: added `transactionRepo.updateIfStatus(id, expectedStatus, data)` (`updateMany` with the old status in the `WHERE` clause), and the loser (`count === 0`) now skips `recalculateContributionStatus` and every downstream ledger/notification/audit step entirely, logging that it lost the race instead. Proved via revert: reverted to the old unconditional `update`, watched the new race-loss test fail (asserting `updateMany` was called — got 0 calls, since the reverted code calls `update`), restored, 35/35 clean. Same fix shape as the goal-payment double-charge fix (PR #339) already in this document — this is the transaction-webhook path having the same class of gap the goal-payment path was hardened against, closed the same way. | 2026-08-29 |
