...
| 4. Confirm successful provider response | `IN PROGRESS` | First attempt returned a real BulkSMS validation error (not a "not configured" error — proving credentials work): `items[0].userSuppliedId size must be between 1 and 20`. Fixed locally (document 2 §6.e, `shortSuppliedId`), proved via revert, restored, 21/21 tests pass — **but not yet committed, pushed, or deployed.** All of this session's fixes (SMS included) exist only in the local working tree on `main`; Vercel production is still running the old code. The row has re-exhausted its 3 retries against that old code and needs a reset **after** this fix actually ships, not before. |
| 5. Confirm Xkimi Xa Mali records the notification correctly | `NOT STARTED` | Depends on step 4, which depends on deployment. |
| 6. Requeue an appropriate failed notification | `BLOCKED` | Blocked on deployment, not credits — resetting now would just fail against the still-live old code. The other 114 are additionally held back pending a credit top-up — see §21.1's SMS row. |
| 7. Confirm successful delivery | `NOT STARTED` | Depends on step 4. |
