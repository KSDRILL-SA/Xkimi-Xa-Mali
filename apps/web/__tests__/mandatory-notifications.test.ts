...
describe('a member cannot switch off news that their money stopped', () => {
  ...
  it('never lists the operational alert templates — they are in-app only, not opt-out-able channels', () => {
    // By owner decision, operational alerts to admins never leave the inbox:
    // no SMS, no email, whatever the severity. See `services/alert.service.ts`.
    // Listing either slug here would only matter if something still queued it.
    expect(MANDATORY_SLUGS.has('admin-alert-email')).toBe(false)
