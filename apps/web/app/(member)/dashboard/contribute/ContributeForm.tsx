    ...
    <div className="max-w-lg space-y-6">
      ...
      {mandate && (
        <>
          ...
          {/* ── What are you paying? ───────────────── */}
          {/*
              The choice only appears when there is somewhere else for the money
              to go — with no active goals there is nothing to choose between.
              But hiding the question used to hide the ANSWER too, and those are
              not the same thing: the page went straight to "Payment details"
              and a member could reach the button without the screen ever
              saying what the money was for. The line below says it in that
              case, so the page always states its purpose even when it has
              nothing to ask.
          */}
          {goals.length === 0 && !success && (
            <div className="bg-white rounded-2xl border border-xxm-green/8 shadow-xxm-sm p-5 flex items-start gap-3">
              <div className="w-9 h-9 rounded-xl bg-xxm-green-50 flex items-center justify-center shrink-0">
                <Wallet size={16} className="text-xxm-green" aria-hidden />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-xs text-xxm-gray-500 mt-0.5">
                  It settles the period you choose below and goes to the Foundation&apos;s fund.
                  There are no other goals open to pay toward right now.
                </p>
              </div>
            </div>
          )}
          ...
          {destination === 'contribution' && (success ? (
            ...
            <div className="bg-white rounded-2xl border border-xxm-green/8 shadow-xxm-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-xxm-gray-100 bg-xxm-green-50/30">
                {/* Named, not "Payment details". A heading that does not say
                    what is being paid is how a member reaches the button
                    without knowing — which is the complaint this answers. */}
                <h2 className="text-base font-bold text-xxm-green-900">Monthly contribution</h2>
                <p className="text-xs text-xxm-gray-500 mt-0.5">
                  Settles the period below, and goes to the Foundation&apos;s fund.
                </p>
              </div>
              <div className="p-5">
                {serverError && <Alert variant="error" className="mb-4">{serverError}</Alert>}
